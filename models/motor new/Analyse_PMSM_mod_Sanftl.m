...
function [Analyse] = Analyse_PMSM_mod(handles)
...
betr.mot.M_max_vec = 1.5.*rated.p.*(((emag.L_d - emag.L_d/2*ctrl.mot.i_1/800) - (emag.L_q - emag.L_q/2*ctrl.mot.i_1/800)).*ctrl.mot.i_1.^2.*0.5.*sin(2.*ctrl.mot.beta) + emag.Psi_PM.*ctrl.mot.i_1.*sin(ctrl.mot.beta));
...
ctrl.mot.i_1_pk = sqrt(ctrl.mot.i_1d_mesh.^2+ctrl.mot.i_1q_mesh.^2);
...
% ctrl.mot.u_1q_mesh = emag.R_1.*ctrl.mot.i_1q_mesh +
% betr.mot.omega_k_mesh.*(emag.L_d.*ctrl.mot.i_1d_mesh + emag.Psi_PM);
ctrl.mot.u_1d_mesh = emag.R_1.*(1+1.8.*(betr.mot.f_el_mesh./max(betr.mot.f_el_mesh,[],'all')).^2).*ctrl.mot.i_1d_mesh - betr.mot.omega_k_mesh.*(emag.L_q - emag.L_q/2*ctrl.mot.i_1_pk/800).*ctrl.mot.i_1q_mesh;
ctrl.mot.u_1q_mesh = emag.R_1.*(1+1.8.*(betr.mot.f_el_mesh./max(betr.mot.f_el_mesh,[],'all')).^2).*ctrl.mot.i_1q_mesh + betr.mot.omega_k_mesh.*((emag.L_d - emag.L_d/2*ctrl.mot.i_1_pk/800).*ctrl.mot.i_1d_mesh + emag.Psi_PM);
...
if(opt.Generator)
    ...
    betr.gen.M_max_vec = 1.5.*rated.p.*(((emag.L_d - emag.L_d/2*ctrl.mot.i_1/800) - (emag.L_q - emag.L_q/2*ctrl.mot.i_1/800)).*ctrl.gen.i_1.^2.*0.5.*sin(2.*ctrl.gen.beta) + emag.Psi_PM.*ctrl.gen.i_1.*sin(ctrl.gen.beta));
    ...
    ctrl.gen.i_1_pk = sqrt(ctrl.gen.i_1d_mesh.^2+ctrl.gen.i_1q_mesh.^2);
    % Berechnen der Spannungen (Stator) fuer Punkte im Drehzahl-/Drehmomentgitter
    ...
    ctrl.gen.u_1d_mesh = emag.R_1.*(1+1.8.*(betr.gen.f_el_mesh./max(betr.gen.f_el_mesh,[],'all')).^2).*ctrl.gen.i_1d_mesh - betr.gen.omega_k_mesh.*(emag.L_q - emag.L_q/2*ctrl.gen.i_1_pk/800).*ctrl.gen.i_1q_mesh;
    ctrl.gen.u_1q_mesh = emag.R_1.*(1+1.8.*(betr.gen.f_el_mesh./max(betr.gen.f_el_mesh,[],'all')).^2).*ctrl.gen.i_1q_mesh + betr.gen.omega_k_mesh.*((emag.L_d - emag.L_d/2*ctrl.gen.i_1_pk/800).*ctrl.gen.i_1d_mesh + emag.Psi_PM);
...
if(opt.P_vw==1)
    loss.P_vw_mesh = 1.5 * emag.R_1 * (ctrl.i_1d_mesh.^2 + ctrl.i_1q_mesh.^2) .*(1+1.8.*(betr.f_el_mesh./max(betr.f_el_mesh,[],'all')).^2);
else
...
if(opt.P_vu==1)
    ...
    elseif(strcmp(opt.P_vu_Modell,'Abschaetzung ueber verketteten Fluss'))
        ...
        loss.vu.c_Fe2 = 1.6; % zwischen 1.5 und 1.6
        
        % Berechnen der Eisenverluste
        loss.P_vu_mesh = loss.vu.c_Fe1.*betr.omega_k_mesh.^(loss.vu.c_Fe2).*(((emag.L_d-emag.L_d/2*sqrt(ctrl.i_1d_mesh.^2+ctrl.i_1q_mesh.^2)/800).*ctrl.i_1d_mesh + emag.Psi_PM).^2 + ((emag.L_q-emag.L_q/2*sqrt(ctrl.i_1d_mesh.^2+ctrl.i_1q_mesh.^2)/800).*ctrl.i_1q_mesh).^2);
    elseif(strcmp(opt.P_vu_Modell,'Sanftl'))
        loss.P_vu_mesh = 1e-10.*betr.omega_k_mesh.^2.5 .* sqrt(ctrl.i_1d_mesh.^2+ctrl.i_1q_mesh.^2).^1.5 + 0.15.*betr.omega_k_mesh.^1; 
    else
...
if(opt.P_vme==1)
    ...
    loss.vme.k_rb = 8; % sollte zwischen 5 und 15 liegen
...
%L_q = emag.L_q - emag.L_q/2*sqrt(x(1)^2 + x(2)^2)/800;
%L_d = emag.L_d - emag.L_d/2*sqrt(x(1)^2 + x(2)^2)/800;

if(Generator)
    % Minimierungsfunktion 
    % fun = @(x) -1.5.*rated.p.*((emag.L_d-emag.L_q).*-x(1).^2.*0.5.*sin(2.*x(2)) + emag.Psi_PM.*-x(1).*sin(x(2)));
    fun = @(x) -1.5.*rated.p.*(((emag.L_d - emag.L_d/2*x(1)/800) - (emag.L_q - emag.L_q/2*x(1)/800)).*-x(1).^2.*0.5.*sin(2.*x(2)) + emag.Psi_PM.*-x(1).*sin(x(2)));
    % Startwert
    x0 = [0 0];
    % Bound Constraints
    lb = [0 -pi];
...
%   Nonlinear Equality Constraint: keine
    ...
    c = (R_1*x(1)*cos(x(2)) - omega_k*(L_q - L_q/2*x(1)/800)*x(1)*sin(x(2)))^2 + (R_1*x(1)*sin(x(2)) + omega_k*(L_d - L_d/2*x(1)/800)*x(1)*cos(x(2)) + omega_k*Psi_PM)^2 - u_1max^2;
    ceq = [];
end
        ...
        if(isnan(M))
            ...
            if(j>1 && ~isnan(i_1dsol(k,j-1)) && ~isnan(i_1qsol(k,j-1)))
                if(k>1 && ~isnan(i_1dsol(k-1,j)))
                    ...
                    @(x) nonlcon(x, M, opt.u_1max, opt.i_1max, emag.L_d, emag.L_q, emag.R_1, omega_k, emag.Psi_PM, rated.p, omega_max), options);
                else
                    % Minimierung mit Startwerten aus vorheriger Berechnung
...
function [c,ceq] = nonlcon_Optimierung_i(x, M_ref, u_1max, i_1max, L_d, L_q, R_1, omega_k, Psi_PM, p, omega_max)
%nonlcon_fun Definition der nichtlinearen Nebenbedingungen
...
%   Nonlinear Equality Constraint: Momentenreferenz
    ...
    c = [x(1)^2 + x(2)^2 - i_1max^2;...
        (R_1.*(1+1.8.*(omega_k/omega_max).^2)*x(1)-omega_k*(L_q- L_q/2*sqrt(x(1)^2 + x(2)^2)/800)*x(2))^2 + (R_1.*(1+1.8.*(omega_k/omega_max).^2)*x(2)+omega_k*(L_d - L_d/2*sqrt(x(1)^2 + x(2)^2)/800)*x(1)+omega_k*Psi_PM)^2 - u_1max^2];
    ceq = 1.5*p*((L_d - L_d/2*sqrt(x(1)^2 + x(2)^2)/800) - (L_q - L_q/2*sqrt(x(1)^2 + x(2)^2)/800))*x(2)*x(1) + 1.5*p*Psi_PM*x(2) - M_ref;
end
