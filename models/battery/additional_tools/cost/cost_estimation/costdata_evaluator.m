function [costdata] = costdata_evaluator(battery_GD)
% costdata_evaluator_testversion v1.0
...
%% Initialization
h.cell_type = ["Cyl" "round";"Cylindrical" "round";"Pouch" "pouch";"Prismatic" "prysmatic"];

% cleartrg_capacity, trg_cellchemistry
% clc

exl = actxserver('Excel.Application');

exl.Visible = 0;    % Open Excel GUI                     

exlWkbk = exl.Workbooks;
filepath = (string(cd) + '\models\battery\additional_tools\cost\cost_estimation\cost_estimation_drivetrain.xlsx');
...
% for API use
trg.capacity = battery_GD.SysInfo.E_sys;
...
var.cellchemistry = battery_GD.BatPara.cellchemistry; % For test mode
if contains(var.cellchemistry, "NMC")
    if contains(var.cellchemistry, "111")
        var.cellchemistry = "NMC-111";
    elseif contains(var.cellchemistry, "811")
        var.cellchemistry = "NMC-811";
    elseif contains(var.cellchemistry, "721")
        var.cellchemistry = "NMC-721";
    elseif contains(var.cellchemistry, "622")
        var.cellchemistry = "NMC-622";
    end
end

var.format = h.cell_type(find(ismember(battery_GD.BatPara.cell_type,h.cell_type(:,1))),2);
...
for i_format = 1:size(var.format,1)
    ...
    for i_cellchemistry = 1:size(var.cellchemistry,1)
        ...
        for i_country = 1:size(var.country,1)
            ...
            %calculates cost in [€/kWh]
            % costdata.(strfield(var.country(i_country)))=...
            %     rng.batterycost.Value()/rng.capacity.Value();
        
            %calculates cost in [€]
             costdata.(strfield(var.country(i_country)))=...
                rng.batterycost.Value()/rng.capacity.Value()* battery_GD.SysInfo.E_sys; 
        end
    end
end
