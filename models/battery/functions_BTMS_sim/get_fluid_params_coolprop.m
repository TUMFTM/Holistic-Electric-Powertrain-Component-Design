...
% Using Python Bridge vor CoolProp 6.4.1 - Modded for GlobalDrive Project

fluid_struct.kin_vis = py.CoolProp.CoolProp.PropsSI('VISCOSITY','T',mean_fluid_temp,'P',ref_pressure,fluid_type)/py.CoolProp.CoolProp.PropsSI('DMASS','T',mean_fluid_temp,'P',ref_pressure,fluid_type);      % kinematic viscosity in[m^2/s]
fluid_struct.lambda  = py.CoolProp.CoolProp.PropsSI('CONDUCTIVITY','T',mean_fluid_temp,'P',ref_pressure,fluid_type);                                                                             % Thermal conductivity in [W/mK]
fluid_struct.rho     = py.CoolProp.CoolProp.PropsSI('DMASS','T',mean_fluid_temp,'P',ref_pressure,fluid_type);                                                                                    % Density in [kg/m^3]
fluid_struct.cp      = py.CoolProp.CoolProp.PropsSI('CPMASS','T',mean_fluid_temp,'P',ref_pressure,fluid_type);                                                                                   % specific heat capacity [J/kgK]
fluid_struct.Pr      = py.CoolProp.CoolProp.PropsSI('PRANDTL','T',mean_fluid_temp,'P',ref_pressure,fluid_type);                                                                                  % Prandtl number
