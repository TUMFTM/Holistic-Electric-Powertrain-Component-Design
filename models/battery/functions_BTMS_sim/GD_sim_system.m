%function results = GD_sim_system(battery_config, GD_config)
function [BatPara, BTMSConfig, mod_ID, ModInfo, SimPara, sys_ID, SysInfo, SysPara, SysSpec] = GD_sim_system(battery_config,GD_config)
...
%% expand config
[sys_ID, mod_ID, SysSpec, BatPara, SysPara, ~, ModInfo, SysInfo, BTMSConfig] = expand_config(battery_config);
...
%%% Quickcharge Cycle
SimPara.t_step               = 0.01;    % basevalue: 0.01
SimPara.t_sim                = 3*3600;  % basevalue: 3*3600
...
% Set to zero if there is no heat transfer in one or more spacial
% directions.

SysPara.thermal.transfer.alpha_x = 1000;       % thermal transmittance between the cells in x-direction in W/(m^2*K)
SysPara.thermal.transfer.alpha_y = 1000;       % thermal transmittance between the cells in y-direction in W/(m^2*K)
SysPara.thermal.transfer.alpha_z = 1000;       % thermal transmittance between the cells in z-direction in W/(m^2*K)

SysPara.thermal.transfer.alpha_mod_gap = 100;  % heat transfer coefficient between the cells in all direction between different modules in W/(m^2*K)
...
% influences.
if GD_config.simulation_parameter_deviation == 0
...
% Initial state of every individual cell inside the system (hence the
...
BatPara.physical = rmfield(BatPara.physical,'TabLocation');
...
model_R = 'sim_BTMS';   % for setting up the Modelworkspace
model = 'BEV_Laengsdynamikmodell_Reiter';

load_system(model)
load_system(model_R);
mod_wrksp = get_param(model_R,'ModelWorkspace');
