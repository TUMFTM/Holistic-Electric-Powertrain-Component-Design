function [P_V, total_P_VD, total_P_VZ, total_P_VL] = Getrieberechnung_EinstufigFW(Gearbox,Mrad,nrad,Temp)
% Funktion zur Verlustberechnung eines 1-stufigen Strinradgetriebes mit
...
for k = 1:3
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% Stufe 1 (radseitig + getriebeseitig)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
    ...
    % Drehzahlen und Drehmomente in Stufe 1 berechnen 
    Welle(2).n = nrad;
    ...
    Stufe(1).P = (Welle(2).n*2*pi/60)*M(2); %übertragene Leistung in Stufe 1
    ...
    etaOil = rho*ny/1e3; %dynamische Viskosität des Öls
    ...
    P_VQ(1) = T_Q * Welle(2).n *2*pi/60; %= 0, da T_Q = 0
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% Lagerkräfte Welle 2 (Siehe Kraftplan im Anhang der SA)
    a = 80+b/2; c = 20+b/2; %Extraabstand für Differential
    Welle(2).L1Fax = Fax;
    Welle(2).L2Fax = 0;
    ...
    %% Lagerkräfte Welle 1 (Siehe Kraftplan im Anhang der SA)
    a = 8+b/2; c =8+b/2; 
    Welle(1).L1Fax = -Fax;
    Welle(1).L2Fax = 0;
    ...
    for i = 1:2 %Iteration über Wellen
    ...
    Welle(2).P_V = P_VD(2) + sum(P_VL(3:4));
...
% disp(['Getriebeverlustleistung Gesamt: ',sprintf('%0.2f',sum(P_VD)+sum(P_VZ)+sum(P_VL)),' W'])
