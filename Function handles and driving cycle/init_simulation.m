classdef init_simulation
    methods(Static)
        ...
        function batteryInitFlag = init_battery(bat_size_factor)
            ...
            battery = Framework_sim_BTMS_interface();
            %battery = sim_BTMS_interface(battery, config_battery, config_vehicle); % call interface
            assignin('base', 'battery', battery)

            % Load battery config - make path more modular using existing path variable
            h.A = load(fullfile(pathes.bat.Batterymodel, config_battery.libraryname, config_battery.libraryname + "_param_dch.mat"));
            h.f = fieldnames(h.A);
            clear(h.f{:})
